...
#'                        ms1=ms1,ms2=ms2)
#' }
MMPrdM<-function(pheno,geno,env,para,Para_Name,depend=NULL,reshuffle=NULL,impute="TRUE",
      ...
      for(i in 1:reshuffle){
        ...
        yobs.whole.cross=numeric()
          ...
          if(model == "rrBLUP"){
            ...
            cv.fit<-glmnet::glmnet(y=yNa[id.T],x=genotype[id.T,-1],
            ...
            lambda_min <- cv.fit$lambda.min
            cv.fit<-glmnet::glmnet(y=yNa[id.T],x=genotype[id.T,-1],
                                      alpha=ENalpha,lambda=lambda_min)
            G.pred<-Marker[id.V,]
            GEBV.inter<- stats::predict(cv.fit,newx=G.pred,s=c(lambda_min))
            #coef<-coef(cv.fit)
            #e=as.matrix(coef@x[-1])
          ...
          #print("OK2")
          t<-c(1:length(id.V))+length(id.V)*(i-1)
          Prd[t,]<-yhat.whole.cross

          Obs <- pheno[id.V,-1]
          Prd <- Prd %>% select(colnames(Obs))
          #print(i)
          Prd<-as.data.frame(Prd)
          for(j in 1:n.para)
          {cor.within[i,j]=cor(Prd[t,j],Obs[,j],use = "complete.obs")}
          #print(cor.within)
          #print(Prd)
        #colnames(yhat.whole.cross)<-colnames(pheno)
      }
      colnames(cor.within)<-colnames(Obs)
      Prd$line_code<-rep(pheno[id.V,1],reshuffle)
      Obs_final <- pheno[id.V,]
      Prd_mean <- Prd %>% group_by(line_code) %>% summarize_all(mean)
      Prd_mean <- Prd_mean %>% select(colnames(Obs_final))
      Prd_mean <- as.data.frame(Prd_mean)
      #Correlation within environment 50 times
      return(list(Prd=Prd_mean,Obs=Obs_final,e_cor=cor.within))
    }
    if(methods=="RM.GE"){
      genotype.match=match(pheno[,1],geno[,1])
      genotype_1=geno[genotype.match,];
      ...
      if(model == "rrBLUPJ" | model == "RRJ"){
        ...
        for(r in 1:reshuffle){
          ...
          for (i in 1:n.line) {
            ...
            #need improve
            ObsM_wide<- y_Prd %>% tidyr::pivot_wider(
              id_cols = line_code,
              names_from = Envs,
              values_from = Obs
            )
            ObsM_wide<-as.data.frame(ObsM_wide)
            #yhat.whole.cross <- rbind(yhat.whole.cross,yNas)
            #print(cor.all)
            for(k in 1:n.para){
            cor.within[r,k]<-cor(PrdM_wide[,k+1],
                                 ObsM_wide[,k+1],
                                 use = "complete.obs")
            }
            #return(cor.within)
        ...
        print(dim(ObsM_wide))
        print(head(Prd_mean))
        print(head(ObsM_wide))
        cor_f<-data.frame(nrow=1,ncol=n.para)
        print(cor_f)
        print("OK")
        for(k in 1:n.para){
          cor_f[1,k]<-cor(Prd_mean[,k+1], ObsM_wide[,k+1],
                               use = "complete.obs")
        }
        colnames(cor_f)<-colnames(Prd_mean[,-1])
        print("OK2")
        return(list(Obs=ObsM_wide,Prd=Prd_mean,cor=cor.within,cor_final=cor_f))
        #Correlation within environment 50 times
      }
      if(methods=="RM.GE"){
        cor.within=matrix(999,reshuffle,n.para);cor.all=numeric();
