...
#' @param env_cols Environmental colors gradient; if NULL,
#' it generates colors using rainbow_hcl.
...
#'
#' @return A plot of etl, filling the area between env-trait q25 to q75,
#' with a red line representing the env-trait mean.
#' @export
#'
#' @importFrom colorspace rainbow_hcl
#' @importFrom ggplot2 element_text geom_line geom_point
...
etl_plotter <- function(data, trait, env_cols = NULL, shape = NULL,
                        size = NULL, linewidth = NULL, area_color = NULL,
                        area_alpha = NULL) {
  ...
  p <-p + labs(x = "env", y = "Trait") +
    geom_point(data = trait, aes(x = trait[,1], y = trait[,2]),
               color = env_cols, size = size, shape = shape,
               show.legend = FALSE) +
    ...
    geom_line(data = trait, aes(x = trait[,1], y = trait[,4], group = 1),
              color = "black", linewidth = linewidth, show.legend = FALSE) +
    geom_ribbon(data = trait, aes(x = trait[,1], ymin = trait[,3],
                                  ymax = trait[,4], group = 1),
