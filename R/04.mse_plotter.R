...
#' @export
#'
#' @importFrom ggplot2 aes geom_text geom_point element_blank theme_bw theme unit
#'
#' @examples
#' \dontrun{mse_plotter(MSE)}
mse_plotter<-function(data,point_size=NULL,text_size=NULL,point_shape=NULL){
  ...
  ggplot(data)+
    geom_point(aes(data[,3], data[,2], color = data[,1]),size=point_size,shape=point_shape) +
    geom_text(aes(data[,3], data[,2], label = data[,1]), size = text_size,hjust=-1,vjust=1) +
