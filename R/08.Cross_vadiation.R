...
#' @importFrom dplyr %>% mutate group_by row_number select
        ...
        for(r in 1:reshuffle){
          ...
          for(f in 1:fold)
          {
            ...
            if (model == "RF"){
              ...
              M <- M[-missing_rows_index, ]
              print(dim(M))
              print(dim(yNa))
              Prd<-randomForest::randomForest(M, y=yNa[,ncol(yNa)],
                                              xtest=N)$test$predicted
            ...
            if (model == "rrBLUP"){
              ...
              dtest <- lightgbm::lgb.Dataset.create.valid(dtrain, N,
                                                          label = y_Prd[,ncol(y_Prd)])
              valids <- list(test = dtest)
          ...
          #need improve
          yobs.whole.cross <- ys
          yobs.whole.crossl<-tidyr::gather(yobs.whole.cross,
                                           key = "Envs",
                                           value = "Obs",
                                           -line_code)
          yhat.whole.cross <- yhat.whole.cross[order(factor(yhat.whole.cross$line_code,
                                                            levels = ys[,1])), ]
          #yhat.whole.cross <- rbind(yhat.whole.cross,yNas)
          yhat.whole.cross <- stats::aggregate(. ~ line_code, data = yhat.whole.cross,
                                               FUN = mean)
          yhat.whole.cross <- yhat.whole.cross[order(factor(yhat.whole.cross$line_code,
                                                            levels = yobs.whole.cross[,1])), ]
          yhat.whole.cross <- yhat.whole.cross %>% select(colnames(yobs.whole.cross))
          yhat.whole.crossl<-tidyr::gather(yhat.whole.cross,
                                           key = "Envs",
                                           value = "Pre",
                                           -line_code)
